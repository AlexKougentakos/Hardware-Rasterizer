...
namespace dae
{
	struct Camera
	{
		...
		void Update(const Timer* pTimer)
		{
			...
			if (pKeyboardState[SDL_SCANCODE_LSHIFT]) sprintMultiplier = 3.f;

			if (pKeyboardState[SDL_SCANCODE_W] || pKeyboardState[SDL_SCANCODE_UP])
			{
				origin += forward * movementSpeed * sprintMultiplier * deltaTime;
			}
			...
			{
				origin += right * movementSpeed * sprintMultiplier * deltaTime;
			}
